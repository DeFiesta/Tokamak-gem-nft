...
async function execAllowance(contract: any, fromSigner: Signer, toAddress: string, amount: BigNumber) {
    let allowance = await contract.allowance(fromSigner.getAddress(), toAddress);
...
describe('New Simple Staking Test', () => {
    ...
    describe('DepositManager v1 ', () => {
        it('deposit will be reverted ', async () => {
            ...
            await (await deployed.TON.connect(deployer).transfer(addr1.getAddress(), tonAmount)).wait()
        ...
        it('deposit(address,uint256) will be reverted ', async () => {
            ...
            await (await deployed.WTON.connect(deployer).transfer(addr1.getAddress(), wtonAmount)).wait()

            const beforeBalance = await deployed.WTON.balanceOf(addr1.getAddress());
    ...
    describe('basic functions ', () => {
        it('deposit to level19 using approveAndCall', async () => {
            ...
            let tonAmount = ethers.utils.parseEther("100")
            await deployed.TON.connect(deployer).transfer(addr1.getAddress(), tonAmount);

            const beforeBalance = await deployed.TON.balanceOf(addr1.getAddress());
            ...
            const afterBalance = await deployed.TON.balanceOf(addr1.getAddress());
        ...
        it('deposit to tokamak using approveAndCall', async () => {
            ...
            expect(roundDown(stakedB.add(BigNumber.from(2)), 1)).to.be.eq(
                roundDown(stakedA.add(tonAmount.mul(BigNumber.from("1000000000"))), 1)
            )
        ...
        it('deposit to level19 using deposit(address,uint256)', async () => {
            ...
            let wtonAmount = ethers.utils.parseEther("100" + "0".repeat(9))
            await deployed.WTON.connect(deployer).transfer(addr1.getAddress(), wtonAmount);
            ...
            const afterBalance = await deployed.WTON.balanceOf(addr1.getAddress());
        ...
        it('deposit to tokamak using deposit(address,address,uint256) ', async () => {
            ...
            const beforeSenderBalance = await deployed.WTON.balanceOf(deployer.getAddress());
            ...
            const afterSenderBalance = await deployed.WTON.balanceOf(deployer.getAddress());
        ...
        it('deposit to tokamak using deposit(address,address[],uint256[]) ', async () => {
            ...
            let stakedA1 = await deployed.seigManagerV2["stakeOf(address,address)"](layer2Info_tokamak.layer2, addr1.getAddress())
            let stakedA2 = await deployed.seigManagerV2["stakeOf(address,address)"](layer2Info_tokamak.layer2, addr2.getAddress())
            ...
            let stakedB1 = await deployed.seigManagerV2["stakeOf(address,address)"](layer2Info_tokamak.layer2, addr1.getAddress())
            let stakedB2 = await deployed.seigManagerV2["stakeOf(address,address)"](layer2Info_tokamak.layer2, addr2.getAddress())

            expect(roundDown(stakedB1.add(BigNumber.from(2)), 1)).to.be.eq(
                roundDown(stakedA1.add(wtonAmount), 1)
            )
            expect(roundDown(stakedB2.add(BigNumber.from(2)), 1)).to.be.eq(
                roundDown(stakedA2.add(wtonAmount), 1)
            )
        })
        ...
        it('requestWithdrawal to level19', async () => {
            ...
            const beforeBalance = await deployed.WTON.balanceOf(account.getAddress())

            let stakedA = await deployed.seigManagerV2["stakeOf(address,address)"](layer2, account.getAddress())
            let pendingUnstakedA = await deployed.depositManagerV2.pendingUnstaked(layer2, account.getAddress())
            let pendingUnstakedLayer2A = await deployed.depositManagerV2.pendingUnstakedLayer2(layer2)
            let pendingUnstakedAccountA = await deployed.depositManagerV2.pendingUnstakedAccount(account.getAddress())
            ...
            const afterBalance = await deployed.WTON.balanceOf(account.getAddress());
            expect(afterBalance).to.be.eq(beforeBalance)

            let stakedB = await deployed.seigManagerV2["stakeOf(address,address)"](layer2Info_level19.layer2, account.getAddress())

            expect(roundDown(stakedA.sub(BigNumber.from(2)), 1)).to.be.eq(
                roundDown(stakedB.add(wtonAmount), 1)
            )

            expect(
                await deployed.depositManagerV2.pendingUnstaked(layer2, account.getAddress())
            ...
            expect(
                await deployed.depositManagerV2.pendingUnstakedAccount(account.getAddress())
        ...
        it('processRequest to level19.', async () => {
            let layer2 = layer2Info_level19.layer2
            let account = addr1
            const beforeBalance = await deployed.TON.balanceOf(account.getAddress())
            let pendingUnstakedA = await deployed.depositManagerV2.pendingUnstaked(layer2, account.getAddress())
            let pendingUnstakedLayer2A = await deployed.depositManagerV2.pendingUnstakedLayer2(layer2)
            let pendingUnstakedAccountA = await deployed.depositManagerV2.pendingUnstakedAccount(account.getAddress())

            let accUnstakedA = await deployed.depositManagerV2.accUnstaked(layer2, account.getAddress())
            let accUnstakedLayer2A = await deployed.depositManagerV2.accUnstakedLayer2(layer2)
            let accUnstakedAccountA = await deployed.depositManagerV2.accUnstakedAccount(account.getAddress())
            ...
            await (await deployed.depositManagerV2.connect(account).processRequest(
                layer2,
                true
            )).wait()

            const afterBalance = await deployed.TON.balanceOf(account.getAddress());
            ...
            expect(
                await deployed.depositManagerV2.accUnstaked(layer2, account.getAddress())
            ...
            expect(
                await deployed.depositManagerV2.accUnstakedAccount(account.getAddress())
    ...
    describe('updateSeigniorage', () => {
        ...
        it('updateSeigniorageAll ', async () => {
            ...
            for (i = 0; i < num; i++) {
                ...
                await (await deployed.seigManagerV2.connect(deployer).updateSeigniorageLayer(layer2)).wait()
            }
        ...
        it('deposit to level19', async () => {
            ...
            let account = addr1
            let tonAmount = ethers.utils.parseEther("100")
            await deployed.TON.connect(deployer).transfer(account.getAddress(), tonAmount);
            ...
            let stakedB = await deployed.seigManagerV2["stakeOf(address,address)"](layer2, account.getAddress())
    ...
    describe('snapshot functions', () => {
        ...
        it('snapshot()', async () => {
            let layer2 = layer2Info_level19.layer2
            let account = addr1
            snapshotInfo.account = account.getAddress()
            snapshotInfo.totTotalSupply = await deployed.seigManagerV2.stakeOfTotal()
            snapshotInfo.accountBalanceOfLayer2 = await deployed.seigManagerV2["stakeOf(address,address)"](layer2, account.getAddress())
            snapshotInfo.accountBalanceOfTotal = await deployed.seigManagerV2["stakeOf(address)"](account.getAddress())
        ...
        it('deposit to level19  ', async () => {
            ...
            let wtonAmount = ethers.utils.parseEther("100" + "0".repeat(9))
            await deployed.WTON.connect(deployer).transfer(account.getAddress(), wtonAmount);
        ...
        it('snapshot data is accurate ', async () => {
            ...
            let accountBalanceOfTotal = await deployed.seigManagerV2["stakeOf(address)"](account.getAddress())
            let totTotalSupply = await deployed.seigManagerV2.stakeOfTotal()

            let accountBalanceOfTotalAt = await deployed.seigManagerV2["stakeOfAt(address,uint256)"](account.getAddress(), snapshotInfo.snapshotId)
            let totTotalSupplyAt = await deployed.seigManagerV2.stakeOfTotalAt(snapshotInfo.snapshotId)
        ...
        it('requestWithdrawal to level19', async () => {
            ...
            await deployed.depositManagerV2.connect(account).requestWithdrawal(
                layer2Info_level19.layer2,
                wtonAmount
            );
